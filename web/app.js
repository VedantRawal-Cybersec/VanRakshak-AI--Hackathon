...
function selectedFilters(){const rawCloud=Number($('cloudFilter')?.value||50);return {fresh:$('freshnessFilter')?.value||'',res:Number($('resolutionFilter')?.value||0),cloud:Math.max(0,Math.min(100,Number.isFinite(rawCloud)?rawCloud:50)),start:$('startDate')?.value||'',end:$('endDate')?.value||'',confidence:$('confidenceFilter')?.value||'high'}}
function allLayers(){return state.layers.flatMap(g=>g.layers||[])}
function findLayerBy(predicate){return allLayers().find(predicate)}
function layerMatchesFilters(l,f=selectedFilters()){return (!f.fresh||l.freshness===f.fresh)&&(!f.res||!l.resolution_m||Number(l.resolution_m)<=f.res)}
function satelliteLayerPath(mode='true_color'){const f=selectedFilters(),q=new URLSearchParams({lat:String(state.lat),lon:String(state.lon),mode,cloud_lt:String(f.cloud)});if(f.start)q.set('start_date',f.start);if(f.end)q.set('end_date',f.end);return '/api/map/satellite-layer?'+q.toString()}
function renderLayers(){if(!$('layerGroups'))return;const f=selectedFilters();$('layerGroups').innerHTML=state.layers.map(g=>{const rows=(g.layers||[]).filter(l=>layerMatchesFilters(l,f)).map(l=>`<div class="layer-row"><label><input type="checkbox" data-layer="${esc(l.id)}" ${state.active.has(l.id)?'checked':''}> ${esc(l.label)}</label><span class="layer-meta">${esc(l.source||'')}<br>${l.resolution_m?esc(l.resolution_m)+' m ':''}${esc(l.freshness||'')}</span></div>`).join('');return rows?`<div class="layer-group"><h3>${esc(g.icon||'')} ${esc(g.label)}</h3>${rows}</div>`:''}).join('');$$('[data-layer]').forEach(x=>x.onchange=()=>toggleLayer(findLayerBy(l=>l.id===x.dataset.layer),x.checked,x))}
...
async function applyLayerFilters(){const f=selectedFilters();if(f.start&&f.end&&f.start>f.end){toast('Filter error: start date must be before end date');return}const activeDefs=allLayers().filter(d=>state.active.has(d.id));for(const d of activeDefs){if(!layerMatchesFilters(d,f))clearDynamicLayer(d.id)}renderLayers();for(const d of activeDefs){if(layerMatchesFilters(d,f)&&['satellite','gfw','gibs'].includes(d.render))await toggleLayer(d,true,null)}try{if(state.active.has('quick-satellite')){const d=await api(satelliteLayerPath('true_color'));addRaster('quick-satellite',d.tile_url,.88)}if(state.active.has('quick-ndvi')){const d=await api(satelliteLayerPath('ndvi'));addRaster('quick-ndvi',d.tile_url,.78)}}catch(e){toast('Filter refresh: '+String(e.message).slice(0,150))}}
...
async function toggleLayer(def,enabled,checkbox){if(!def)return;if(!enabled){clearDynamicLayer(def.id);return}const f=selectedFilters();try{
  if(def.render==='satellite'){const s=await api(satelliteLayerPath(def.mode||'true_color'));addRaster(def.id,s.tile_url,def.mode==='true_color'?.9:.74);toast(`${def.label} loaded from ${s.source||'satellite source'}`)}
...
async function quickLayer(kind){$$('.map-pill').forEach(x=>x.classList.toggle('active',x.dataset.quick===kind));try{
  if(kind==='satellite'){
    try{const d=await api(satelliteLayerPath('true_color'));addRaster('quick-satellite',d.tile_url,.88);toast('Latest suitable Sentinel-2 imagery loaded')}
    catch(primaryErr){const s=await api('/api/gibs/layer/viirs_snpp_true_color?date='+encodeURIComponent(selectedFilters().end||isoDate(new Date(Date.now()-86400000))));addRaster('quick-satellite',s.tile_url,.88);toast('Sentinel-2 unavailable for filters • NASA VIIRS true-color fallback loaded')}
  }
  if(kind==='ndvi'){
    try{const d=await api(satelliteLayerPath('ndvi'));addRaster('quick-ndvi',d.tile_url,.78);toast('Sentinel-2 NDVI layer loaded')}
    catch(primaryErr){const s=await api('/api/gibs/layer/modis_terra_ndvi_8day?date='+encodeURIComponent(selectedFilters().end||isoDate(new Date(Date.now()-86400000))));addRaster('quick-ndvi',s.tile_url,.78);toast('Sentinel-2 NDVI unavailable for filters • NASA MODIS NDVI fallback loaded')}
  }
  if(kind==='fire'){
...
async function runPatrol(){try{if(!ensureLocation())return;const before=$('beforeDate').value,after=$('afterDate').value;if(!before||!after)throw new Error('Select real before/after dates first');$('patrolResult').textContent='Building patrol stops from real Sentinel-2 change polygons…';const q=new URLSearchParams({lat:String(state.lat),lon:String(state.lon),place:state.place,before_date:before,after_date:after,max_points:'5'});const d=await api('/api/patrol/live?'+q.toString());$('patrolResult').textContent=JSON.stringify(d,null,2);if(d.road_route?.geometry){clearDynamicLayer('patrol-route');map.addSource('src-patrol-route',{type:'geojson',data:{type:'Feature',properties:{},geometry:d.road_route.geometry}});map.addLayer({id:'lyr-patrol-route',type:'line',source:'src-patrol-route',paint:{'line-color':'#37e79c','line-width':4,'line-opacity':.95}});state.active.set('patrol-route',{source:'src-patrol-route',layer:'lyr-patrol-route'});fitSelected()}toast('Patrol route built from real detected-change polygons')}catch(e){$('patrolResult').textContent=e.message}}
