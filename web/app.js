...
function addRaster(id,tileUrl,opacity=.72,opts={}){removeLayer(id);const source=`src-${id}`,layer=`lyr-${id}`;const install=()=>{if(map.getSource(source)||map.getLayer(layer))removeLayer(id);const src={type:'raster',tiles:[tileUrl],tileSize:Number(opts.tileSize||256)};if(Number.isFinite(Number(opts.maxzoom)))src.maxzoom=Number(opts.maxzoom);if(Number.isFinite(Number(opts.minzoom)))src.minzoom=Number(opts.minzoom);if(opts.attribution)src.attribution=opts.attribution;map.addSource(source,src);map.addLayer({id:layer,type:'raster',source,paint:{'raster-opacity':opacity,'raster-fade-duration':120}},map.getLayer('labels')?'labels':undefined);state.active.set(id,{source,layer,tileUrl});};if(map.isStyleLoaded())install();else map.once('load',install)}
