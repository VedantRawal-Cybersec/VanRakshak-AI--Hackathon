...
- **Resilient provider chain:** multi-sensor **NASA FIRMS** when its free key is configured, plus no-key **NASA EONET + GIBS** fire/imagery fallbacks, **Open-Meteo**, **SoilGrids**, **OSM/Overpass**, **Protected Planet v4 + OSM protected-area fallback**, **Nominatim + Photon geocoding fallback**, **GDELT**, **ISRO Bhuvan**, and **MOSDAC helpers**.
...
| 2 | Forest Cover Monitoring | `WORKING_WITH_PUBLIC_FALLBACK` | /api/layers<br>/api/map/gfw-layer<br>/api/earth-engine/layer/dynamic_world_trees |
...
| 6 | Multi-Layer Earth Map | `WORKING` | /api/layers<br>/api/map/gfw-layer<br>/api/gibs/layer/{layer_id}<br>/api/earth-engine/layer/{layer_id}<br>/api/bhuvan/tile/{z}/{x}/{y}.png |
...
| 11 | Fire & Heat Detection | `WORKING_WITH_PUBLIC_FALLBACK` | /api/fire<br>/api/fire/intelligence<br>/api/eonet/events<br>/api/gibs/catalog |
...
| 25 | Protected Area Intelligence | `WORKING_WITH_PUBLIC_FALLBACK` | /api/protected-area/context<br>/api/protected-areas<br>/api/protected-areas/{site_id}/parcels |
...
| 35 | Live Command Center | `WORKING` | /<br>/api/health<br>/api/source-health<br>/api/fallbacks/status |
...
`WORKING_WITH_PUBLIC_FALLBACK` means the product function stays operational through a scientifically labelled public fallback when an optional credentialed source is unavailable. VanRakshak never relabels fallback context as equivalent evidence.
...
- **FIRMS_MAP_KEY** — upgrades fire intelligence to NOAA-21 + NOAA-20 + MODIS pixel-level NRT detections. Without it, EONET/GIBS fallback context remains operational.
- **PROTECTED_PLANET_TOKEN** — upgrades protected-area intelligence to official Protected Planet v4. Without it, WDPA (when EE is available) or OSM protected-area containment is used and labelled as fallback.
- **Earth Engine credentials/project** — optional enhancement for Dynamic World, GEDI, SRTM, MODIS, CHIRPS, WDPA and related layers. Public GFW/Sentinel/NASA fallbacks keep core workflows usable without it.
...
GET  /api/satellite/landsat
GET  /api/map/satellite-layer
GET  /api/map/compare
GET  /api/map/gfw-layer
GET  /api/gibs/catalog
GET  /api/gibs/layer/{layer_id}
...
GET  /api/soil
GET  /api/fire
GET  /api/fire/intelligence
GET  /api/fire/history
GET  /api/protected-area/context
GET  /api/fallbacks/status
...
PYTHONPATH=backend python scripts/acceptance_check.py
PYTHONPATH=backend python scripts/verify_public_providers.py
